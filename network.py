...
class Network(object):
    ...
    def print_bandusage(self):
        ...
        if total > 0:
            print("TV:", self.band_usage[0] / total, "ISM:", self.band_usage[1] / total, "LTE:",
                  self.band_usage[2] / total, "CBRS:", self.band_usage[3] / total)
    ...
    def load_primary_users(self):  # loads a pre generated file of primary users and how they should operate
        ...
        if dataset == "UMass":
            with open("Primary_Users/" + dataset + "/" + str(puser_round) + "/primary_usersUMass_" + str(
                    num_channels) + ".txt", "r") as f:
                lines = f.readlines()
        elif dataset == "Lexington":
            with open("Primary_Users/" + dataset + "/" + str(puser_round) + "/primary_usersLEX_" + str(
                    num_channels) + ".txt", "r") as f:
                lines = f.readlines()
    ...
    def fill_network(self, num_nodes):  # quickly fill network and randomly place nodes for testing purposes
        for i in range(num_nodes):  # create and add nodes to network
            ide = str(i)
            node = Node(ide)
            # TODO: Address this once we start dealing with Lexington data
            # node.load_pkl()
            self.add_node(node)
    ...
    def get_message_info(self, path_lines, spec_lines, src, des, t,
                         size):  # finds a msg's predetermined path (xchants only)
        path = []
        band = []
    ...
    def messages_delivered(
            self):  # creates a file of all delivered packets and their meta data so metrics can be calculated
    ...
    def save_packets_per_tau(self):  # creates file for metrics
        with open(path_to_metrics + "packets_per_tau.txt", "w") as f:
            ...
            for i in range(T):
                f.write(
                    str(i) + "\t" + str(self.packets_per_tau_list[i]) + "\t" + str(self.parallel_coms_list[i]) + "\n")
        ...
        for line in lines:
            ...
            if int(line_arr[5]) == time:
                ...
                for j in range(num_packets):
                    new_mes = Message(line_arr[0], line_arr[1], line_arr[2], line_arr[5], line_arr[4], [0, 0, 0, 0], -1,
                                      -1, 0, j, 0)
                    new_mes.create_copies(num_replicas)
                    src = int(line_arr[1])
                    self.nodes[src].buf.append(new_mes)

    def not_delivered_messages(
            self):  # creates file for metrics of packets that are in the network that haven't been delivered
        for node in self.nodes:
            for mes in node.buf:
        ...
        for msg_id in range(len(msg_lines)):
            ...
            if (int(msg_line_arr[5]) == t):  # if a new message needs to be generated at this time
                ...
                for packet_id in range(num_packets):
                    ...
                    message = Message(id, src, des, t, size, [0, 0, 0, 0], path_list, band_list, 0, packet_id,
                                      0)  # create the message
    ...
    def get_node_fwd_priority(self, nodes_in_range, msg,
                              t):  # finds which nodes should be prioritized in geographic routing
    ...
    def q_learning_get_next_node_and_band(self, node):
        if np.random.rand() <= max(node.epsilon, node.epsilon_min):
            action_index = random.randrange(len(node.action_space))
        else:
            action_index = np.argmax(node.q_table)
        node.epsilon = node.epsilon * node.epsilon_decay
        return action_index
        ...
        for node in self.nodes:
            ...
            for s in range(numSpec):
                ...
                for neighbouring_node in nodes_in_range:
                    if (int(neighbouring_node.ID), s) not in node.action_space:
    ...
    def network_GO(self, t, specBW, path_lines, spec_lines, msg_lines,
        ...
        # Q learning protocol for DDSA network
        elif protocol == "q_learning":
                    ...
                    for msg in node.buf:
                        if msg.last_sent < t:
                            ...
                            if node_priority_list != -1 and node_priority_list[0] != node:
                                ...
                                if node.mes_fwd_time_limit <= (num_sec_per_tau * num_transceivers):
                                    msg_sent, num_packet_broadcasted = node.try_broadcasting_message_epi(
                                        nodes_to_broadcast, msg, t, LINK_EXISTS,
